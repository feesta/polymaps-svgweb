...
po.layer = function(load, unload) {
  ...
  function move() {
    ...
    function scanLine(x0, x1, y) {
      ...
      for (var x = x0; x < x1; x++) {
				var coord = {column: x, row: y, zoom: z};
        var t = cache.load(coord, projection);
...
function scanTriangle(a, b, c, ymin, ymax, scanLine) {
  ...
  // scan span! scan span!
	var scanned = false;
  if (ab.dy) { scanSpans(ca, ab, ymin, ymax, scanLine); scanned = true; }
  if (bc.dy) { scanSpans(ca, bc, ymin, ymax, scanLine); scanned = true; }
}
po.image = function() {
...
po.drag = function() {
  ...
  function mousemove(e) {
    if (!dragging) return;
		var delta = {x: e.clientX - dragging.x, y: e.clientY - dragging.y};
    map.panBy(delta);
